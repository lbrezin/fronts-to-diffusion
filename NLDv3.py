...
class pd:
    """Class that implements exponential growth, nonlinear biomass diffusion, and nutrient diffusion in one dimension."""
    def __init__(self, param):
        ...
        # upper diagonal
        self.dm[0,2:] = -self.cnf*np.ones(self.N-2)
        ...
        # lower diagonal
        self.dm[2,:-2] = -self.cnf*np.ones(self.N-2)
    ...
    def biomass_diffusion(self):
        ...
        if self.method == 'diffusive':
            ...
            #Left flux
            Jl = (np.power(self.n[:-2],self.alpha)*np.power(self.b[:-2],self.beta) + np.power(self.n[1:-1],self.alpha)*np.power(self.b[1:-1],self.beta))/2*(self.b[1:-1]-self.b[:-2])
            #Right flux
            Jr = (np.power(self.n[2:],self.alpha)*np.power(self.b[2:],self.beta) + np.power(self.n[1:-1],self.alpha)*np.power(self.b[1:-1],self.beta))/2*(self.b[2:]-self.b[1:-1])
            self.b[1:-1] += self.bdf*(Jr-Jl) # main update
            self.b[0] = self.b[1] # boundary conditions
            self.b[-1] = self.b[-2] # boundary conditions
    ...
    def nutrient_diffusion(self):
        ...
        #Neumann
            ...
            raise ValueError("Unsupported BC. Only 'Neu' and 'Dir' are currently supported")
        rhs[1:-1] += self.cnf*(self.n[2:]+self.n[:-2]) # bulk
...
def compute_front(b_array, dx, threshold):
    ...
    if i == 0: # if the first index is at the boundary, then no linear interpolation can occur
        print("Warning: Index at starting boundary.")
...
def collect_biomass_data(param, step=100,threshold_fraction=0.05):
    ...
    while condition:
        ...
        sim.step(step)
        lt.append(sim.t)
        ltb.append(trapezoid(sim.b*sim.dx))
...
def sqrtfit_dataset(data, min_len = 0.5):
    ...
    for index,elem in enumerate(data):
        time = elem['time']
        biomass = elem['total_biomass']
        slope, intercept, (start, end), r2 = automatic_fit(np.power(time,0.5), biomass, r2_threshold=0.8, min_len=int(min_len*len(biomass)))
        results = {
            "xi" : slope,
            "r2" : r2
        }
